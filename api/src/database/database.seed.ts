...
export const runSeed = async (): Promise<void> => {
	...
	try {
		...
		const fixedCostTypesCounts = await instance.fixedCostType.count();

		if (fixedCostTypesCounts == 0) {
		...
		const operationCategoryCounts = await instance.operationCategory.count();
		if (operationCategoryCounts == 0) {
